...
(define-map id-to-participant uint {addr: principal, value: uint})
(define-data-var participants uint u0)
(define-data-var is-locked bool false)
(define-data-var registered-funds uint  u0)
(define-map cycle-to-winner uint principal)
;; private functions
  ...
  (begin 
    (let
      ( 
        ...
        (winner (get addr (unwrap-panic (map-get? id-to-participant (unwrap-panic (element-at final-list (unwrap-panic vrf )))))))
      )
      (ok winner)
    )
  )
...
(define-private (register-new-user (add (tuple (hashbytes (buff 20)) (version (buff 1)))))
  (begin
    (asserts! (or (is-eq 0x00 (get version add)) (is-eq 0x01 (get version add)) (is-eq 0x02 (get version add)) (is-eq 0x03 (get version add))) NOT_VALID_REGISTRATION)
    ...
    (if (< (var-get participants) u150)
      (begin
        ...
        (map-set id-to-participant (var-get participants) {addr: tx-sender, value: u0})
        (var-set participants (+ (var-get participants) u1))
        (ok (map-set stx-to-btc {stx-address: tx-sender} {btc-address: add}))
      )
      (let
        (
        ...
        (if (> (len open-id-list) u0)
          (begin
            ...
            (map-set id-to-participant (unwrap-panic (map-get? participant-to-id tx-sender)) {addr: tx-sender, value: u0})
            (ok (map-set stx-to-btc {stx-address: tx-sender} {btc-address: add}))
          )
...
(define-public (change-btc-credentials (add (tuple (hashbytes (buff 20)) (version (buff 1)))))
  (begin
    (asserts! (or (is-eq 0x00 (get version add)) (is-eq 0x01 (get version add)) (is-eq 0x02 (get version add)) (is-eq 0x03 (get version add))) NOT_VALID_REGISTRATION)
    (asserts! (is-eq tx-sender contract-caller) CC_TX_SENDER_DONT_MATCH)
...
(define-public (deposit-stacks (amount uint) (addre (optional (tuple (hashbytes (buff 20)) (version (buff 1))))) )
  (begin 
    ...
    (if (is-some (map-get? stx-to-btc { stx-address: tx-sender }))
      (begin 
        ...
        (map-set id-to-participant (unwrap-panic (map-get? participant-to-id tx-sender)) {addr: tx-sender, value: (+ (get value (unwrap-panic (map-get? id-to-participant (unwrap-panic (map-get? participant-to-id tx-sender))))) amount)} )
        (var-set registered-funds (+ (var-get registered-funds) amount))
      )
      (let (
        (cred (unwrap! addre PROVIDE_VALID_TUPLE ))
      ) 
        (asserts! (is-ok (register-new-user cred)) NOT_VALID_REGISTRATION)
        (map-set id-to-participant (unwrap-panic (map-get? participant-to-id tx-sender)) {addr: tx-sender, value: (+ (get value (unwrap-panic (map-get? id-to-participant (unwrap-panic (map-get? participant-to-id tx-sender))))) amount)} )
        (var-set registered-funds (+ (var-get registered-funds) amount))
      )
    )
    (asserts! (is-ok (stx-transfer? (* amount (pow u10 u6)) tx-sender (as-contract tx-sender))) STX_TRANSFER_FAILED)
    (if (is-eq (var-get registered-funds) u150000)
...
(define-public (stake-stx (burn-height uint)) 
  (begin 
    (asserts! (is-eq tx-sender deployer) NOT_VALID_SENDER)
    (asserts! (is-eq (var-get is-locked) true) CONTRACT_NOT_LOCKED)
    (let
      (
        (winner (unwrap-panic (get-winner)))
        (tr (unwrap-panic (contract-call? 'ST000000000000000000002AMW42H.pox get-pox-info)))
      )
      (begin 
        (map-set cycle-to-winner (+ (get reward-cycle-id tr) u1) winner)
      )
      (if (>= (stx-get-balance (as-contract tx-sender)) u150000000000)
        (match (as-contract (contract-call? 'ST000000000000000000002AMW42H.pox stack-stx (stx-get-balance (as-contract tx-sender)) (get btc-address (unwrap-panic (map-get? stx-to-btc {stx-address: winner}))) burn-height number-of-cycles))
          worked (ok u100)
        ...
        (let
          ...
          (match (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.vault provideToPool amount)
            worked (begin
              ...
              (map-set id-to-participant u9999 {addr: 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.vault, value: amount})
              (match (as-contract (contract-call? 'ST000000000000000000002AMW42H.pox stack-stx (stx-get-balance (as-contract tx-sender)) (get btc-address (unwrap-panic (map-get? stx-to-btc {stx-address: winner}) )) burn-height number-of-cycles))
                pass (ok u1000)
                error (err (to-uint error))
              )
...
(define-read-only (id-to-part (id uint)) 
    (map-get? id-to-participant  id)
)
...
(define-read-only (get-next-burn-height) 
  (let
      (
        (tr (unwrap-panic (contract-call? 'ST000000000000000000002AMW42H.pox get-pox-info)))
        (burn-height (+ (* (get reward-cycle-length tr) (+ (get reward-cycle-id tr) u1)) (get first-burnchain-block-height tr)))
      )
      burn-height
  )
)
...
(define-read-only (get-cycle-winner (cycle uint)) 
  (map-get? cycle-to-winner cycle)
)

(define-read-only (check-locked) 
  (var-get is-locked)
)