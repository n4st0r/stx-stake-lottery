...
(define-map participant-to-id-per-cycle {cycle: uint, address: principal} {id: uint})
(define-map id-to-participant-per-cycle {cycle: uint, id: uint} {address: principal, value: uint})
(define-map cycle-to-participants uint uint)
(define-map cycle-to-is-locked uint bool)
(define-map cycle-to-balance uint uint)
...
(define-private (is-greater-than-zero (id uint))
    (let 
        (
            (next-cycle (+ u1 (get reward-cycle-id (unwrap-panic (contract-call? 'ST000000000000000000002AMW42H.pox get-pox-info)))))
        )
    (match (map-get? id-to-participant-per-cycle {cycle: next-cycle, id: id})
...
(define-private (get-winner) 
  (begin 
    (let
      ...
      (ok (get address (unwrap-panic (map-get? id-to-participant-per-cycle {cycle: next-cycle, id: (unwrap-panic (element-at final-list (unwrap-panic vrf )))}))))
    )
  )
)
...
(define-private (register-new-user (address (tuple (hashbytes (buff 20)) (version (buff 1)))))
    (begin
        ...
        (let
            (
                (next-cycle (+ u1 (get reward-cycle-id (unwrap-panic (contract-call? 'ST000000000000000000002AMW42H.pox get-pox-info)))))
                (participants (unwrap-panic (map-get? cycle-to-participants next-cycle)))
            )
        (if (< participants u1500)
            (begin
                (map-set participant-to-id-per-cycle {cycle: next-cycle, address: tx-sender} {id: participants})
                (map-set id-to-participant-per-cycle {cycle: next-cycle, id: participants} {address: tx-sender, value: u0})
                (map-set cycle-to-participants next-cycle (+ participants u1))
            ...
            (if (> (len open-id-list) u0)
                (begin
                    (map-set participant-to-id-per-cycle  {cycle: next-cycle, address: tx-sender} {id:  (unwrap-panic (element-at open-id-list u0))})
                    (map-set id-to-participant-per-cycle {cycle: next-cycle, id: (get id (unwrap-panic (map-get? participant-to-id-per-cycle {cycle: next-cycle, address: tx-sender})))} {address: tx-sender, value: u0})
                    (ok (map-set stx-to-btc {stx-address: tx-sender} {btc-address: address}))
                )
...
(define-public (change-btc-credentials (address (tuple (hashbytes (buff 20)) (version (buff 1)))))
  (let
    ...
    (asserts! (or (is-eq 0x00 (get version address)) (is-eq 0x01 (get version address)) (is-eq 0x02 (get version address)) (is-eq 0x03 (get version address))) NOT_VALID_REGISTRATION)
    (asserts! (is-eq tx-sender contract-caller) CC_TX_SENDER_DONT_MATCH)
    (asserts! (is-eq (unwrap-panic (map-get? cycle-to-is-locked next-cycle)) false) CONTRACT_IS_LOCKED)
    (asserts! (is-some (map-get? participant-to-id-per-cycle {cycle: next-cycle, address: tx-sender})) NEED_TO_DEPOSIT_FIRST)
...
(define-public (deposit-stacks (amount uint) (address (optional (tuple (hashbytes (buff 20)) (version (buff 1))))) )
  (let
    ...
    (if (is-some (map-get? stx-to-btc { stx-address: tx-sender }))
      (let
        (
            (id (get id (unwrap-panic (map-get? participant-to-id-per-cycle {cycle: next-cycle, address: tx-sender}))))
            (value (get value (unwrap-panic (map-get? id-to-participant-per-cycle {cycle: next-cycle, id: id}))))
        ) 
        (asserts! (<= (+ value amount) u20000) NOT_VALID_AMOUNT)
        (map-set id-to-participant-per-cycle {cycle: next-cycle, id: id} {address: tx-sender, value: (+ value amount)})
      )
      (begin
        (asserts! (is-ok (register-new-user (unwrap! address PROVIDE_VALID_TUPLE))) NOT_VALID_REGISTRATION)
        (map-set id-to-participant-per-cycle {cycle: next-cycle, id: (get id (unwrap-panic (map-get? participant-to-id-per-cycle {cycle: next-cycle, address: tx-sender})))} {address: tx-sender, value: amount})
      )
    )
    (if (is-some (map-get? cycle-to-balance next-cycle))
        (map-set cycle-to-balance next-cycle (+ (unwrap-panic (map-get? cycle-to-balance next-cycle)) amount))
        (map-insert cycle-to-balance next-cycle amount)
    )
...
(define-public (withdraw-stacks (cycle uint)) 
  (begin
    (if (and (> (get value (unwrap-panic (map-get? id-to-participant-per-cycle {cycle: cycle, id: (get id (unwrap-panic (map-get? participant-to-id-per-cycle {cycle: cycle, address: tx-sender})))}))) u0) (not (unwrap-panic (map-get? cycle-to-is-locked cycle))))
      (let
        (
            (id (get id (unwrap-panic (map-get? participant-to-id-per-cycle {cycle: cycle, address: tx-sender}))))
            (amount (get value (unwrap-panic (map-get? id-to-participant-per-cycle {cycle: cycle, id: id}))))
            (receipient tx-sender)
        )
        (map-set id-to-participant-per-cycle {cycle: cycle, id: id} {address: tx-sender, value: u0})
        (map-set cycle-to-balance cycle (- (unwrap-panic (map-get? cycle-to-balance cycle)) amount))
...
(define-public (toggle-lock (cycle uint) (lock bool)) 
  (begin
    (asserts! (is-eq tx-sender deployer) (err u000)) 
    (ok (map-set cycle-to-is-locked cycle lock))
  )
)
...
(define-public (stake-stx) 
  (let 
    ...
    (asserts! (is-eq (unwrap-panic (map-get? cycle-to-is-locked next-cycle)) true) CONTRACT_NOT_LOCKED)
    (let
      (
        ...
        (cycle-balance (unwrap-panic (map-get? cycle-to-balance next-cycle)))
      )
      (match (as-contract (contract-call? 'ST000000000000000000002AMW42H.pox stack-stx (* cycle-balance (pow u10 u6)) (get btc-address (unwrap-panic (map-get? stx-to-btc {stx-address: (unwrap-panic (get-winner))}))) burn-height number-of-cycles))
        worked (ok u100)
