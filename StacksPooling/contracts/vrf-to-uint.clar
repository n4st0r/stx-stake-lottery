...
;; vrf-to-uint
;; <Vrf-seed to uint mod number of participants in order to get the winner of the cycle. Based on citycoins Algorithm>
...
;; public functions
(define-public (get-rnd (participants uint))
  (ok (mod (lower-16-le (unwrap! (get-block-info? vrf-seed (- block-height u1)) ERR_FAIL)) participants))
)
;;
